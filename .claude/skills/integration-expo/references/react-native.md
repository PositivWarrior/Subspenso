...
import { useFeatureFlag } from 'posthog-react-native';
const MyComponent = () => {
	const multiVariantFeature = useFeatureFlag(
		'key-for-your-multivariate-flag',
	);
...
function handleFeatureFlag(client, flagKey, distinctId) {
	try {
		...
		console.error(
			`Error fetching feature flag '${flagKey}': ${error.message}`,
		);
...
const posthog = new PostHog('<ph_project_token>', {
	...
	before_send: (event) => {
		if (event.event === '$exception') {
			...
			const exception =
				exceptionList.length > 0 ? exceptionList[0] : null;
			if (exception) {
